    ...
    contract KernelDebug is EIP712, Compatibility, KernelStorage {
        ...
        fallback() external payable {
            ...
            console2.log("fallback sig:");
            console2.logBytes4(sig);
            console2.log("fallback executor:", executor);
            ...
            returns (ValidationData validationData)
        {
            ...
            if (mode == 0x00000001) {
                ...
                console2.log("validateUserOp userOpCallData:");
                console2.logBytes(userOpCallData);
                ExecutionDetail storage detail = getKernelStorage().execution[bytes4(userOpCallData[0:4])];
                validator = detail.validator;
                console2.log("validateUserOp mode == 0x00000001 validator:", address(validator));
                userOpSignature = userOpSignature[4:];
                validationData = packValidationData(detail.validAfter, detail.validUntil);
                console2.log("validateUserOp mode == 0x00000001 validationData:", ValidationData.unwrap(validationData));
            } else if (mode == 0x00000002) {
                console2.log("mode == 0x00000002");
                bytes calldata userOpCallData;
                assembly {
                    userOpCallData.offset := add(calldataload(add(userOpEndOffset, 0x40)), userOpEndOffset)
                    userOpCallData.length := calldataload(sub(userOpCallData.offset, 0x20))
                ...
                console2.log("validateUserOp SIG_VALIDATION_FAILED");
            ...
            returns (IKernelValidator validator, ValidationData validationData, bytes calldata validationSig)
        {
            unchecked {
                ...
                bytes32 enableDigest = _hashTypedData(
                    keccak256(
                        abi.encode(
                            VALIDATOR_APPROVED_STRUCT_HASH, // constant value
                            bytes4(sig), // selector
                            uint256(bytes32(signature[4:36])), // validatorData: uint48(validAfter) + uint48(validUntil) + address(validator) 
                            address(bytes20(signature[36:56])), // executor
                            keccak256(enableData) // enableDataHash
                        )
                    )
                );
                logBytes32Value("approveValidator EIP712 enable digest:", enableDigest);
        ...
        function validateSignature(bytes32 hash, bytes calldata signature) public view returns (ValidationData) {
            console2.log("------------validateSignature in Kernel.sol");
